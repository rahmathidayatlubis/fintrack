...
@section('header-left')
    <a href="{{ route('accounts.show', $account) }}" class="w-8 h-8 rounded-xl bg-gray-100 flex items-center justify-center">
...
@section('header-title', 'Edit Rekening')

@section('header-right')
    <form action="{{ route('accounts.destroy', $account) }}" method="POST"
        onsubmit="return confirm('Hapus rekening {{ $account->name }}? Aksi ini tidak bisa dibatalkan.')">
        @csrf @method('DELETE')
...
@section('content')
    @if (session('error'))
        <div class="mx-4 mt-4 p-3 rounded-xl bg-red-100 text-red-700 text-sm">
            {{ session('error') }}
        </div>
    @endif
        ...
        <form action="{{ route('accounts.update', $account) }}" method="POST" class="space-y-4">
            ...
            <div class="rounded-3xl p-5 relative overflow-hidden transition-all"
                ...
                <div class="relative z-10">
                    <p class="text-white/70 text-xs font-500">Preview</p>
            ...
            <div class="bg-white rounded-3xl p-4 shadow-card space-y-4">
                <div>
                    <label class="text-xs font-600 text-gray-500 uppercase tracking-wide mb-1.5 block">Nama Rekening
                        *</label>
                    <input type="text" name="name" x-model="cardName" value="{{ old('name', $account->name) }}"
                ...
                <div>
                    <label class="text-xs font-600 text-gray-500 uppercase tracking-wide mb-1.5 block">Jenis *</label>
                    <select name="type"
                        ...
                        @foreach ($types as $key => $type)
                            <option value="{{ $key }}"
                                {{ old('type', $account->type) === $key ? 'selected' : '' }}>{{ $type['label'] }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    <label class="text-xs font-600 text-gray-500 uppercase tracking-wide mb-1.5 block">Nomor
                        Rekening</label>
                    <input type="text" name="account_number" x-model="cardNumber"
                ...
                <div>
                    <label class="text-xs font-600 text-gray-500 uppercase tracking-wide mb-1.5 block">Nama Bank</label>
            ...
            <div class="bg-white rounded-3xl p-4 shadow-card">
                <label class="flex items-center justify-between cursor-pointer">
                    ...
                    <div class="relative">
                        <input type="checkbox" name="include_in_total" value="1" id="include_toggle"
                            {{ $account->include_in_total ? 'checked' : '' }} class="sr-only">
    ...
    @push('scripts')
        <script>
            function accountForm() {
                return {
                    cardName: '{{ old('name', $account->name) }}',
                    cardNumber: '{{ old('account_number', $account->account_number) }}',
                    cardColor: '{{ old('color', $account->color) }}',
                    cardIcon: '{{ old('icon', $account->icon) }}',
                    includeInTotal: {{ $account->include_in_total ? 'true' : 'false' }},
                }
            }
        </script>
    @endpush
