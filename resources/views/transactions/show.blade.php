    ...
    <div class="flex items-center gap-2">
        ...
        @if ($transaction->type !== 'transfer')
            <a href="{{ route('transactions.edit', $transaction) }}"
                ...
                <svg class="w-4 h-4 text-blue-500" fill="none" viewBox="0 0 24 24" stroke-width="2"
                    stroke="currentColor">
...
@section('content')
    <div class="px-4 pt-4">
        ...
        <!-- Amount card -->
        <div
            class="rounded-3xl p-6 mb-4 text-center relative overflow-hidden
        ...
        <div class="bg-white rounded-3xl overflow-hidden shadow-card mb-4 divide-y divide-gray-50">
            ...
            <div class="flex items-center justify-between px-4 py-3.5">
                <span class="text-xs font-600 text-gray-400">
                    @if ($transaction->type === 'transfer' && $transaction->transfer_type === 'debit')
                        Dari Rekening
                    @elseif($transaction->type === 'transfer' && $transaction->transfer_type === 'credit')
                        Ke Rekening
                    @else
                        Rekening
                    @endif
            ...
            @if ($transaction->type === 'transfer' && $transaction->destinationAccount)
                <div class="flex items-center justify-between px-4 py-3.5">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="w-6 h-6 rounded-lg" style="background: {{ $transaction->destinationAccount->color }};">
                        </div>
            ...
            <div class="flex items-center justify-between px-4 py-3.5">
                ...
                <span class="text-sm font-600 text-gray-500 font-mono">Rp
                    {{ number_format($transaction->balance_before, 0, ',', '.') }}</span>
            </div>
            <div class="flex items-center justify-between px-4 py-3.5">
                <span class="text-xs font-600 text-gray-400">Saldo Sesudah</span>
                <span class="text-sm font-700 text-gray-900 font-mono">Rp
            ...
            <div class="flex items-center justify-between px-4 py-3.5">
                ...
                <span
                    class="text-xs font-600 text-gray-400 font-mono">#{{ str_pad($transaction->id, 8, '0', STR_PAD_LEFT) }}</span>
            </div>
        </div>
